...
const ruleName = 'react-exhaustive-deps'
...
function collectRecommendations({
  ...
  let isInsideElmishMemo = false
  let hookParent = reactiveHook.parent
  while (hookParent !== null) {
    if (hookParent.type === 'CallExpression' && hookParent.callee.name === 'ElmishMemo') {
      isInsideElmishMemo = true
      break
    }

    hookParent = hookParent.parent
  }
  ...
  function scanTreeRecursively(node, missingPaths, satisfyingPaths, keyToPath) {
    node.children.forEach((child, key) => {
      ...
      if (child.isUsed && (path !== 'dispatch' || !isInsideElmishMemo)) {
...
module.exports = { name: ruleName, rule }
